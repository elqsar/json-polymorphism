...
package cz.boris.gson.typeadapter;
 ...
 * {
 * 	&#064;code
 * 	RuntimeTypeAdapter&lt;Shape&gt; shapeAdapter = RuntimeTypeAdapter.of(Shape.class,
 * 			&quot;type&quot;);
 * }
 * </pre>
 * 
 ...
 * 			.registerTypeAdapter(Shape.class, shapeAdapter).create();
 * }
 * </pre>
 * 
 ...
 * {
 * 	&#064;code
 * 	RuntimeTypeAdapter&lt;Shape&gt; shapeAdapter = RuntimeTypeAdapterFactory
 * 			.of(Shape.class).registerSubtype(Rectangle.class)
...
public final class RuntimeTypeAdapterFactory<T> implements TypeAdapterFactory {
	...
	public RuntimeTypeAdapterFactory<T> registerSubtype(
			Class<? extends T> type, String label) {
	...
	public <R> TypeAdapter<R> create(Gson gson, TypeToken<R> type) {
		...
		return new TypeAdapter<R>() {
			...
			public R read(JsonReader in) throws IOException {
				...
				TypeAdapter<R> delegate = (TypeAdapter<R>) labelToDelegate
						.get(label);
				if (delegate == null) {
					throw new JsonParseException("cannot deserialize "
							+ baseType + " subtype named " + label
			...
			public void write(JsonWriter out, R value) throws IOException {
				...
				@SuppressWarnings("unchecked")
				// registration requires that subtype extends T
				TypeAdapter<R> delegate = (TypeAdapter<R>) subtypeToDelegate
						.get(srcType);
				if (delegate == null) {
